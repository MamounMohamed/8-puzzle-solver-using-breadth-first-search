...
class Main {
    public static void BreadthFirstSearch(node root) {// we set goal = false , initialize the queue and the visited set , add the root them ;
        boolean goal = false;
        ...
        while (!Queue.isEmpty() && !goal) {
            node current = Queue.remove(); // we get to the first element in the priority queue , add it to the visited , expand all possible movements
            Visited.add(current);
            current.expandNode();
            for (int i = 0; i < current.children.size(); i++) { // we iterate to the children of  the nodes
               ...
               //asks if  queue contains the node currentchild (if yes it will return true else it will return false)
                if (!contains(Queue,currentChild) && !contains(Visited, currentChild)) {// if currentChild isn't in the queue nor the visited we just add it
    ...
    public static boolean contains(Queue<node> x , node y) {// returns if the queue contains a node or not
        for (node z : x) {
